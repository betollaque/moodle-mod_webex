...
/**
 * webex conversion handler. This resource handler is called by moodle1_mod_resource_handler
 */
class moodle1_mod_webex_handler extends moodle1_resource_successor_handler {
    ...
    public function process_legacy_resource($data) {
        ...
        // prepare the new webex instance record
        $url                 = array();
        $url['id']           = $data['id'];
        $url['name']         = $data['name'];
        ...
        $this->fileman = $this->converter->get_file_manager($contextid, 'mod_webex', 'intro');
        $url['intro'] = moodle1_converter::migrate_referenced_files($url['intro'], $this->fileman);

        // write webex.xml
        $this->open_xml_writer("activities/webex_{$moduleid}/webex.xml");
        $this->xmlwriter->begin_tag('activity', array('id' => $instanceid, 'moduleid' => $moduleid,
            'modulename' => 'webex', 'contextid' => $contextid));
        $this->write_xml('webex', $url, array('/webex/id'));
        $this->xmlwriter->end_tag('activity');
        $this->close_xml_writer();

        // write inforef.xml
        $this->open_xml_writer("activities/webex_{$moduleid}/inforef.xml");
